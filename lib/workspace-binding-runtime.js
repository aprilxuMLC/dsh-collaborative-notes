...
// Keep the platform decision in node:path.  A Windows drive-qualified path
// (for example C:\\workspace\\notes) does not begin with the platform
// separator, so separator-prefix checks are not absolute-path checks.
export function isAbsoluteNotesPath(value, pathApi = { isAbsolute }) {
  return typeof value === "string" && pathApi.isAbsolute(value);
}
...
export function createWorkspaceBindingRuntime(ctx, { layers = [], keyGenerator = newItemKey, resolveExactCarry = null } = {}) {
  ...
  async function validateLocation(current, selectedPath, { defaultChoice = false, adopt = false } = {}) {
    if (!isAbsoluteNotesPath(selectedPath)) {
